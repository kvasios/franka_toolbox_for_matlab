...
//
// NOTE: This is a header-only extraction of libfranka's examples_common.{h,cpp}
// MotionGenerator helper, kept here to avoid building/linking an extra "common"
// static library while still reusing the implementation across toolbox targets.
#pragma once
...
/**
 * Generates a joint pose motion to a goal configuration.
 *
 * Adapted from:
...
class MotionGenerator {
 public:
   ...
   */
  inline MotionGenerator(double speed_factor, const std::array<double, 7> q_goal)
      : q_goal_(q_goal.data()) {
   ...
   */
  inline franka::JointPositions operator()(const franka::RobotState& robot_state,
 ...
 private:
  ...
  inline bool calculateDesiredValues(double t, Vector7d* delta_q_d) const {
  ...
  inline void calculateSynchronizedValues() {
        ...
        if (std::abs(delta_q_[i]) <
          ...
          dq_max_reach[i] =
              std::sqrt(4.0 / 3.0 * delta_q_[i] * sign_delta_q[i] *
                        (ddq_max_start_[i] * ddq_max_goal_[i]) / (ddq_max_start_[i] + ddq_max_goal_[i]));
        }
