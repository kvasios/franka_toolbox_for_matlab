function franka_robot_model_coriolis_sfunction(block)
%FRANKA_ROBOT_MODEL_CORIOLIS_SFUNCTION Compute Coriolis force vector c(q,dq) using libfranka Model
%
...
%
%  Inputs:
%    robot_ip   - IP address of the Franka robot (16 ASCII chars, null-terminated)
...
function setup(block)
    ...
    % Inputs: robot_ip(16), q(7), dq(7), I_total(9), m_total(1), F_x_Ctotal(3)
    ...
    % Input 3: dq - Joint velocities (7x1)
    block.InputPort(3).Dimensions  = 7;
    block.InputPort(3).DatatypeID  = 0;  % double
    block.InputPort(3).Complexity  = 'Real';
    block.InputPort(3).DirectFeedthrough = true;

    % Input 4: I_total - Total load inertia (9x1, 3x3 column-major)
    block.InputPort(4).Dimensions  = 9;
    block.InputPort(4).DatatypeID  = 0;  % double
    block.InputPort(4).Complexity  = 'Real';
    block.InputPort(4).DirectFeedthrough = true;

    % Input 5: m_total - Total load mass (1x1)
    block.InputPort(5).Dimensions  = 1;
    block.InputPort(5).DatatypeID  = 0;  % double
    block.InputPort(5).Complexity  = 'Real';
    block.InputPort(5).DirectFeedthrough = true;

    % Input 6: F_x_Ctotal - Total load CoM (3x1)
